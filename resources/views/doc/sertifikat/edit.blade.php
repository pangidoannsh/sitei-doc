...
@section('content')
    <div class="">
        <h2 class="text-center fw-semibold ">Ubah Sertifikat</h2>

        <form action="{{ route('sertif.update', $data->id) }}" method="POST" class="d-flex flex-column gap-3"
                                            ...
                                            @if ($key < $halfIndex)
                                                <div class="form-check">
                                                    <input class="form-check-input dosen-selector" type="checkbox"
                                                        ...
                                                        name="dosen[]"
                                                        {{ in_array($dosen->nip, $data->penerimas->pluck('user_penerima')->toArray()) ? 'checked' : '' }}>
                                    ...
                                    @foreach ($staffs as $staff)
                                        <div class="form-check">
                                            <input class="form-check-input staf-selector" type="checkbox"
                                                ...
                                                name="staf[]"
                                                {{ in_array($staff->username, $data->penerimas->pluck('user_penerima')->toArray()) ? 'checked' : '' }}>
                                                    ...
                                                    @foreach ($angkatans->sortBy('nama') as $mahasiswa)
                                                        <div class="col form-check mb-2">
                                                            <input
                                                                ...
                                                                name="mahasiswa[]"
                                                                {{ in_array($mahasiswa->nim, $data->penerimas->pluck('user_penerima')->toArray()) ? 'checked' : '' }}>
            ...
            <div class="footer-submit">
                <button type="submit" class="btn btn-success">Ubah Sertifikat</button>
...
@push('scripts')
    <script>
        ...
        @if ($data->penerimas->count() > 0)
            @foreach ($data->penerimas as $penerima)
                @if ($penerima->user_penerima == null)
                    createNewInputPenerima("{{ $penerima->nama_penerima }}")
                @endif
            @endforeach
        @else
            createNewInputPenerima()
